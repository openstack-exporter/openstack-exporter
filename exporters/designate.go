...
var knownZoneStatuses = map[string]int{
	"PENDING": 0,
	"ACTIVE":  1,
	"DELETED": 2,
	"ERROR":   3,
...
var knownRecordsetStatuses = map[string]int{
	"PENDING": 0,
	"ACTIVE":  1,
	"DELETED": 2,
	"ERROR":   3,
...
func mapZoneStatus(status string) int {
	return mapStatus(knownZoneStatuses, strings.ToUpper(status))
}

func mapRecordsetStatus(status string) int {
	return mapStatus(knownRecordsetStatuses, strings.ToUpper(status))
}
...
func NewDesignateExporter(config *ExporterConfig, logger *slog.Logger) (*DesignateExporter, error) {
	...
	sched, err := SetupExporter(&e.BaseOpenStackExporter, &e.descs, &designateGraph)
	if err != nil {
		return nil, err
	}
	e.sched = sched
