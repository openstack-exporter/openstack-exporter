...
// endpoints. Masakari resolves the marker against the numeric database id of
// the last item seen on the previous page, so the UUID cannot be used here.
func masakariListQuery(marker int) string {
	v := url.Values{}
	v.Set("limit", strconv.Itoa(masakariPageLimit))
	if marker > 0 {
		v.Set("marker", strconv.Itoa(marker))
	}
	return "?" + v.Encode()
}

// listMasakariSegments returns every failover segment, following pagination
// until an empty page is returned. The server may cap the page size below the
// requested limit (osapi_max_limit), so a short page does not mean the last one.
// Paging also stops if the marker does not advance, so a server that ignores it
// cannot loop forever.
func listMasakariSegments(ctx context.Context, client *gophercloudv2.ServiceClient) ([]masakariSegment, error) {
	...
	for {
		...
		next := resp.Segments[len(resp.Segments)-1].ID
		if next == 0 || next == marker {
			break
		}
		marker = next
	}
