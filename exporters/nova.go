...
// novaServerStatusBaseLabels are the fixed labels of the server_status
// metric, before any NovaMetadataMapping labels are appended.
var novaServerStatusBaseLabels = []string{"id", "status", "name", "tenant_id", "user_id", "address_ipv4",
...
func NewNovaExporter(config *ExporterConfig, logger *slog.Logger) (*NovaExporter, error) {
	...
	sched, err := SetupExporter(&e.BaseOpenStackExporter, &e.descs, &novaGraph,
		WithDynamicMetrics([]string{"server_status"}, func(base *BaseOpenStackExporter) {
			if !base.IsMetricEnabled("server_status") {
				return
			}
			labels := append(slices.Clone(novaServerStatusBaseLabels), config.NovaMetadataMapping.Labels...)
			e.serverStatusDesc = prometheus.NewDesc(
				prometheus.BuildFQName(base.GetName(), "", "server_status"),
				"server_status", labels, nil)
			base.RegisterDesc(e.serverStatusDesc)
		}))
	if err != nil {
		return nil, err
	}
