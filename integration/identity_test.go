...
func TestIdentityIntegration(t *testing.T) {
	...
	t.Run("openstack_identity_up_metric", func(t *testing.T) {
		if !strings.Contains(bodyString, "openstack_identity_up") {
			t.Errorf("Metric 'openstack_identity_up' not found. Status Code: %d, Endpoint: %s\nBody:\n%s", resp.StatusCode, metricsURL, bodyString)
			return
		}
		if !strings.Contains(bodyString, "openstack_identity_up 1") {
			t.Error("openstack_identity_up metric should have value 1 indicating service is up")
		}
