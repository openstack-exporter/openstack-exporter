...
func main() {
	...
	if *cloud == "" && !*multiCloud {
		logger.Error("msg", "openstack-exporter: error: required argument 'cloud' or flag --multi-cloud not provided, try --help")
	}

	logger.Info("msg", "Build context", "build_context", version.BuildContext())

	if *osClientConfig != DEFAULT_OS_CLIENT_CONFIG {
		logger.Debug("msg", "Setting Env var OS_CLIENT_CONFIG_FILE", "os_client_config_file", *osClientConfig)
		os.Setenv("OS_CLIENT_CONFIG_FILE", *osClientConfig)
	}

	if _, err := os.Stat(*osClientConfig); err != nil {
		logger.Error("err", "Could not read config file", "error", err)
		os.Exit(1)
	}
	...
	select {
	case err := <-errChan:
		logger.Error("err", "Shutting down due to error", "err", err)
		cancel()
	case <-sigChan:
...
func cacheBackgroundService(ctx context.Context, services map[string]*bool, errChan chan<- error, logger *slog.Logger) {
	logger.Info("msg", "Start cache background service")
	...
	for {
		...
		case <-ttlTicker.C:
			...
			logger.Info("msg", "Cache TTL flush")
		case <-ctx.Done():
			logger.Info("msg", "Backend service is stopping")
			return
		}
	}
}
...
func startHTTPServer(ctx context.Context, services map[string]*bool, toolkitFlags *web.FlagConfig, errChan chan<- error, logger *slog.Logger) {
	...
	if *multiCloud {
		...
		logger.Info("msg", "openstack exporter started in multi cloud mode (/probe?cloud=)")
		...
		logger.Info("msg", "openstack exporter started in legacy mode")
	...
	if *domainID != "" {
		logger.Info("msg", "Gathering metrics for configured domain ID", "domain_id", *domainID)
	}

	if *tenantID != "" {
		logger.Info("msg", "Gathering metrics for configured tenant ID", "tenant_id", *tenantID)
	}
...
func probeHandler(services map[string]*bool, logger *slog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		logger.Info("msg", "Enabled services", "enabled_services", enabledServices)

		// Get data from cache
		if *cacheEnable {
			if err := cache.WriteCacheToResponse(w, r, cloud, enabledServices, logger); err != nil {
				logger.Error("err", "Write cache to response failed", "error", err)
			}
			return
		}
		...
		for _, service := range enabledServices {
			...
			if err != nil {
				logger.Error("err", "Enabling exporter for service failed", "service", service, "error", err)
				continue
			}
			registry.MustRegister(*exp)
			logger.Info("msg", "Enabled exporter for service", "service", service)
		}
...
func metricHandler(services map[string]*bool, logger *slog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		logger.Info("msg", "Starting openstack exporter version for cloud", "version", version.Info(), "cloud", *cloud)
		logger.Info("msg", "Build context", "build_context", version.BuildContext())
		...
		if enabledExporters == 0 {
			logger.Error("err", "No exporter has been enabled, exiting")
			os.Exit(-1)
		}
