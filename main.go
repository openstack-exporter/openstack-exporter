...
var (
	...
	placementProviderTraitRegex = kingpin.Flag("placement-provider-trait-regex", "Only report placement resource providers traits matching a regex (default to all traits starting with CUSTOM_)").Default("CUSTOM_").Regexp()
)
...
func cacheBackgroundService(ctx context.Context, services []string, cancel context.CancelCauseFunc, logger *slog.Logger) {
	...
	if err := cache.CollectCache(exporters.EnableExporter, *multiCloud, services, *prefix, *cloud, *disabledMetrics, *endpointType, *collectTime, *disableSlowMetrics, *disableDeprecatedMetrics, *disableCinderAgentUUID, *domainID, *tenantID, novaMetadataMapping, *dnsConcurrentCount, *placementProviderTraitRegex, nil, logger); err != nil {
...
func probeHandler(configuredServices []string, logger *slog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, service := range enabledServices {
			exp, err := exporters.EnableExporter(service, *prefix, cloud, *disabledMetrics, *endpointType, *collectTime, *disableSlowMetrics, *disableDeprecatedMetrics, *disableCinderAgentUUID, *domainID, *tenantID, novaMetadataMapping, *dnsConcurrentCount, *placementProviderTraitRegex, nil, logger)
			if err != nil {
